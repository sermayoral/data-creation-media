...
def process(path, year_from, path_move_media):
  if not path_move_media or os.path.normpath(path) != os.path.normpath(path_move_media):
    for fichero in os.listdir(path):
      filename = os.fsdecode(fichero)
      filepath = os.path.join(path, fichero)
      if os.path.isdir(filepath):
        process(filepath, year_from, path_move_media)
        continue
      ...
      if valid_media > 0:
        ...
        if path_move_media:
          if not os.path.exists(path_move_media):
            os.makedirs(path_move_media)
          os.rename(filepath, os.path.join(path_move_media, fichero))
...
def parseArguments():
  ...
  parser.add_argument("-m", "--move-media", help="Path to move media files without creation date", type=str, default=None)
...
if __name__ == '__main__':
  ...
  # Process media
  process('.', args.year_from, args.move_media)
